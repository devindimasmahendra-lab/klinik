...
.table-responsive{width:100%;overflow-x:auto;-webkit-overflow-scrolling:touch;padding-bottom:4px}
table{width:100%;min-width:720px}
.card{overflow:hidden}
.toolbar{display:flex;flex-wrap:wrap;gap:10px}
.btn-sm{padding:10px 14px !important;border-radius:14px !important}
.patient-detail-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}
.info-tile{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);padding:14px;border-radius:20px}
.info-tile .label{font-size:.72rem;color:#94a3b8;margin-bottom:6px}
.info-tile .value{font-weight:700;color:#fff;word-break:break-word}
.action-cell{position:sticky;right:0;background:rgba(7,17,31,.98);backdrop-filter:blur(14px);z-index:2}
...
@media(max-width:768px){
  body{font-size:14px}
  .card{border-radius:24px !important;padding:1rem !important}
  table{min-width:860px}
  th,td{white-space:nowrap}
  .mobile-stack{display:flex !important;flex-direction:column !important;align-items:flex-start !important}
  .mobile-stack-right{width:100%;display:flex;overflow:auto;gap:10px;padding-bottom:2px}
  .hero h3{font-size:1.15rem !important}
}
...
def patient_detail(patient_id):
    ...
    <div class="card" style="margin-top:16px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><h3 style="margin:0">File Hasil USG</h3><span class="badge">{{ files|length }} file</span></div>{% if files %}<div class='table-responsive'><table><thead><tr><th>File</th><th>Tipe</th><th>Ukuran</th><th>Tanggal</th><th class='action-cell'>Aksi</th></tr></thead><tbody>{% for f in files %}<tr><td><strong>{{ f['original_filename'] }}</strong></td><td>{{ file_badge(f['file_ext']) }}</td><td>{{ '%.2f MB'|format((f['file_size'] or 0)/1024/1024) }}</td><td>{{ fmt_dt(f['created_at']) }}</td><td class='action-cell'><div class="toolbar"><a class="btn btn-sm" href="{{ url_for('file_view_auth', upload_id=f['id']) }}" target="_blank">Buka</a><a class="btn btn-sm" href="{{ url_for('patient_file_public', token=patient['access_token'], upload_id=f['id']) }}" target="_blank">Link Pasien</a></div></td></tr>{% endfor %}</tbody></table></div>{% else %}<div class="empty">Belum ada file hasil USG.</div>{% endif %}</div>
    '''
...
def billing_page():
    ...
    <div class="card" style="margin-top:16px">
    <div class="mobile-stack" style="display:flex;justify-content:space-between;align-items:center;gap:10px">
        <h3 style="margin:0">Billing Klinik</h3>
        <div class="mobile-stack-right">
            <span class="badge">{{ rows|length }} transaksi</span>
            <span class="badge">Responsive Billing</span>
        </div>
    </div>
    {% if rows %}
    ...
    <table>
      <thead>
        <tr>
          <th>Pasien</th><th>Item</th><th>Nominal</th><th>Status</th><th>Tanggal</th><th class="action-cell">Aksi</th>
        </tr>
      </thead>
      <tbody>
      {% for r in rows %}
        <tr>
          <td><strong>{{ r['nama_pasien'] }}</strong><div class="small muted">{{ r['nomor_rekam_medis'] }}</div></td>
          <td>{{ r['item_name'] }}<div class="small muted">{{ r['notes'] or '' }}</div></td>
          <td>{{ rupiah(r['amount']) }}</td>
          <td><span class="badge {{ 'paid' if r['status_bayar']=='lunas' else 'unpaid' }}">{{ r['status_bayar'] }}</span></td>
          <td>{{ fmt_dt(r['created_at']) }}</td>
          <td class="action-cell">
            <div class="action-buttons">
            {% if r['status_bayar'] != 'lunas' %}
              <form method="post" action="{{ url_for('billing_mark_paid', billing_id=r['id']) }}">
                <button class="btn btn-primary btn-sm">✔ Lunas</button>
              </form>
            {% else %}
              <span class="badge paid">Sudah Lunas</span>
            {% endif %}
            </div>
          </td>
        </tr>
    ...
    {% else %}<div class="empty">Belum ada billing.</div>{% endif %}
    </div>
    '''
...
@app.route('/billing/<int:billing_id>/paid', methods=['POST'])
@role_required('superadmin', 'admin')
def billing_mark_paid(billing_id):
    conn = db(); cur = conn.cursor()
    cur.execute("UPDATE billing SET status_bayar='lunas', updated_at=? WHERE id=?", (now(), billing_id))
    conn.commit(); conn.close()
